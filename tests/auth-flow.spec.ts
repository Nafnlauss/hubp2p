...
test.describe('Auth Flow Debug', () => {
  test('should login and navigate without redirect loops', async ({ page }) => {
    ...
    page.on('response', response => {
      if (response.status() >= 300 && response.status() < 400) {
        console.log(`[REDIRECT] ${response.status()} from ${response.url()} to ${response.headers()['location'] || 'unknown'}`);
      }
    });
    ...
    try {
      ...
      if (finalUrl.includes('/kyc')) {
        ...
        if (currentUrl !== finalUrl) {
          throw new Error(`Loop detectado! URL mudou de ${finalUrl} para ${currentUrl}`);
        }

        expect(page.url()).toContain('/kyc');
      } else if (finalUrl.includes('/dashboard')) {
        console.log('✅ Redirecionado para Dashboard (usuário com KYC aprovado)');
  ...
  test('should check cookies after login', async ({ page }) => {
    ...
    // Aguardar navegação
    await page.waitForURL(/.*\/(dashboard|kyc)/, { timeout: 10000 });
    ...
    const hasSupabaseCookie = cookies.some(c => c.name.includes('sb-') && c.name.includes('-auth-token'));
